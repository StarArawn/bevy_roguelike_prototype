...
use super::map::{MapLayer, MapSeed, RoadLayer};
...
pub fn generate_road(
    ...
    map_query: Query<(&Map, &MapSeed), (With<MapLayer>, Without<RoadLayer>)>,
    mut road_query: Query<&mut Map, (With<RoadLayer>, Without<MapLayer>)>,
) {
    if let Ok(mut road_layer) = road_query.single_mut() {
        if let Ok((map_layer, _map_seed)) = map_query.single() {            
            ...
            find_road_path(&mut commands, &tile_query, &mut road_layer, &map_layer, &road_points);
...
pub fn find_road_path(
    ...
    for road_point in road_path.iter() {
        let has_no_tile = road_layer.get_tile(IVec2::new(road_point.0, road_point.1)).is_none();
        ...
        if has_no_tile {
            ...
            road_layer.add_tile(commands, UVec2::new(road_point.0 as u32, road_point.1 as u32), tile, true).unwrap();
        }
    }
...
pub fn generate_map(
    mut commands: Commands,
    mut game_state: ResMut<State<GameState>>,
    mut map_query: Query<(Entity, &mut Map), With<MapLayer>>,
) {
    if let Ok((map_entity, mut map_layer)) = map_query.single_mut() {
